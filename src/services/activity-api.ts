import type { ServerTableQuery, ServerTableResult } from "@/components/data-table-server";
...
const eventTemplates = [
	...
	{ event: "Webhook delivery failed", target: "integrations", level: "critical" },
...
export async function fetchActivity(
	...
	if (query.query.trim()) {
		...
		rows = rows.filter((row) =>
			`${row.event} ${row.user} ${row.target} ${row.level}`.toLowerCase().includes(q)
		);
	}
