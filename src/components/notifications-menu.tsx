...
export function NotificationsMenu() {
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger
				render={
					<Button aria-label={`Notifications (${unreadCount} unread)`} size="icon" variant="ghost" className="relative" />
				}
			>
				<BellIcon />
				{unreadCount > 0 && (
					<span className="absolute -top-0.5 -right-0.5 flex h-3.5 min-w-3.5 items-center justify-center rounded-full bg-red-500 px-0.5 font-medium text-[9px] leading-none text-white">
						{unreadCount}
					</span>
				)}
			</DropdownMenuTrigger>
